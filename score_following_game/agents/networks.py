...
class ScoreFollowingNetMSMDLCHSDeepDoLight(nn.Module):
    ...
    def __init__(self,  n_actions, perf_shape, score_shape):
        ...
        # spec part
        self.spec_conv1 = nn.Conv2d(perf_shape[0], out_channels=16, kernel_size=(3, 3), stride=(1, 1))
        self.spec_conv2 = nn.Conv2d(self.spec_conv1.out_channels, out_channels=16, kernel_size=(3, 3), stride=(1, 1),)

        self.spec_conv3 = nn.Conv2d(self.spec_conv2.out_channels, out_channels=32, kernel_size=(3, 3), stride=(2, 2))
        self.spec_conv4 = nn.Conv2d(self.spec_conv3.out_channels, out_channels=32, kernel_size=(3, 3), stride=(1, 1))
        self.spec_do4 = nn.Dropout(p=0.2)

        self.spec_conv5 = nn.Conv2d(self.spec_conv4.out_channels, out_channels=64, kernel_size=(3, 3), stride=(2, 2))

        self.spec_conv6 = nn.Conv2d(self.spec_conv5.out_channels, out_channels=96, kernel_size=(3, 3), stride=(2, 2))
        self.spec_conv7 = nn.Conv2d(self.spec_conv6.out_channels, out_channels=96, kernel_size=(1, 1), stride=(1, 1))
        self.spec_do7 = nn.Dropout(p=0.2)

        self.spec_fc = nn.Linear(2016, 512)

        # sheet part
        self.sheet_conv1 = nn.Conv2d(score_shape[0], out_channels=16, kernel_size=(5, 5), stride=(1, 2))
        self.sheet_conv2 = nn.Conv2d(self.sheet_conv1.out_channels, out_channels=16, kernel_size=(3, 3), stride=(1, 1))

        self.sheet_conv3 = nn.Conv2d(self.sheet_conv2.out_channels, out_channels=32, kernel_size=(3, 3), stride=(2, 2))
        self.sheet_conv4 = nn.Conv2d(self.sheet_conv3.out_channels, out_channels=32, kernel_size=(3, 3), stride=(1, 1))
        self.sheet_do4 = nn.Dropout(p=0.2)

        self.sheet_conv5 = nn.Conv2d(self.sheet_conv4.out_channels, out_channels=32, kernel_size=(3, 3), stride=(2, 2))

        self.sheet_conv6 = nn.Conv2d(self.sheet_conv5.out_channels, out_channels=64, kernel_size=(3, 3), stride=(2, 2))
        self.sheet_do6 = nn.Dropout(p=0.2)

        self.sheet_conv7 = nn.Conv2d(self.sheet_conv6.out_channels, out_channels=96, kernel_size=(3, 3), stride=(2, 2))
        self.sheet_conv8 = nn.Conv2d(self.sheet_conv7.out_channels, out_channels=96, kernel_size=(1, 1), stride=(1, 1))
    ...
    def forward(self, perf, score):
        ...
        spec_x = spec_x.view(spec_x.shape[0], -1)  # flatten
        ...
        sheet_x = sheet_x.view(sheet_x.shape[0], -1)  # flatten
        ...
        return {'policy': policy, 'value': critic}
...
class ScoreFollowingNetNottinghamLS(nn.Module):
    ...
    def __init__(self,  n_actions, perf_shape, score_shape):
        ...
        self.spec_conv1 = nn.Conv2d(perf_shape[0], out_channels=16, kernel_size=(3, 3), stride=(2, 2))
        self.spec_conv2 = nn.Conv2d(self.spec_conv1.out_channels, out_channels=32, kernel_size=(3, 3), stride=(2, 2))
        self.spec_conv3 = nn.Conv2d(self.spec_conv2.out_channels, out_channels=32, kernel_size=(3, 3), stride=(2, 2))
        self.spec_conv4 = nn.Conv2d(self.spec_conv3.out_channels, out_channels=64, kernel_size=(3, 3), stride=(1, 1))

        self.sheet_conv1 = nn.Conv2d(score_shape[0], out_channels=16, kernel_size=(5, 5), stride=(1, 2))
        self.sheet_conv2 = nn.Conv2d(self.sheet_conv1.out_channels, out_channels=32, kernel_size=(3, 3), stride=(2, 2))
        self.sheet_conv3 = nn.Conv2d(self.sheet_conv2.out_channels, out_channels=32, kernel_size=(3, 3), stride=(2, 2))
        self.sheet_conv4 = nn.Conv2d(self.sheet_conv3.out_channels, out_channels=64, kernel_size=(3, 3), stride=(1, 2))
...
def get_network(network_name, n_actions, shapes=dict(), activation=None):
    """ Compile network by name """
    constructor = eval(network_name)
