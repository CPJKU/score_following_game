...
class A2CAgent(object):
    ...
    def train(self, max_updates=5000, log_writer=None, log_interval=100, evaluator=None, eval_interval=5000,
        ...
        for i in range(1, max_updates+1):
            ...
            # calculate returns
            returns[-1] = self.model.forward_value([Variable(obs[-1]) for obs in observations]).data
            ...
            policy, values = self.model([Variable(obs[:-1].view(-1, *obs.size()[2:])) for obs in observations])
