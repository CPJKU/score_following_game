...
import torch
import numpy as np
from reinforcement_learning.algorithms.networks import PolicyNet, ValueNet
...
def make_env(rank, seed=1, env_id='CartPole-v0'):
    env = gym.make(env_id)
    env.seed(seed + rank)
    return MultiModalityWrapper(env)


def get_make_env(rank, seed=1, env_id='CartPole-v0'):
    def _thunk():
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Discrete Example.')

    parser.add_argument('--agent', help='reinforcement learning algorithm [reinforce|a2c|ppo].', type=str,
                        default="reinforce")

    parser.add_argument('--env', help='the environment', type=str, default='CartPole-v0')
    ...
    policy_net = PolicyNet(input_shape=in_shape, n_actions=env.action_space.n)
    value_net = ValueNet(input_shape=in_shape)

    policy_optimizer = torch.optim.Adam(params=policy_net.parameters(), lr=1e-3)
    value_optimizer = torch.optim.Adam(params=value_net.parameters(), lr=2e-3)
    optimizer = [policy_optimizer, value_optimizer]

    model = SeparatedModel(policy_net=policy_net, value_net=value_net, optimizer=optimizer)
    ...
    if args.agent == 'reinforce':
        agent = ReinforceAgent(env=env, model=model, no_baseline=False, gamma=0.95, use_cuda=args.use_cuda)
    elif args.agent == 'a2c':
        agent = A2CAgent(env=env, model=model, n_worker=args.n_worker, gamma=0.95, use_cuda=args.use_cuda)
    elif args.agent == 'ppo':
        agent = PPOAgent(env=env, model=model, n_worker=args.n_worker, gamma=0.95, use_cuda=args.use_cuda)
    else:
