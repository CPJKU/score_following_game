...
from score_following_game.experiment_utils import get_make_env, make_env_tismir, setup_evaluation_parser, \
    load_game_config
...
from score_following_game.agents.models import Model, dict_obs_to_tensor
...
if __name__ == "__main__":
    ...
    parser.add_argument('--agent_type', help='which agent to test [rl|optimal].',
                        choices=['rl', 'optimal'], type=str, default="rl")
    ...
    env = make_env_tismir(pool, config, render_mode='video', seed=args.seed)
    if args.agent_type == 'optimal':

        agent = OptimalAgent(pool)
        alignment_errors, action_sequence, observation_images, episode_reward = agent.play_episode(env, "video")
        ...
        if args.plot_ig:
            ...
            plain_env = get_make_env(copy.deepcopy(pool), config, make_env_fnc=make_env_tismir, render_mode='video', seed=args.seed)()
        ...
        while True:
            ...
            if env.obs_image is not None:
                ...
                if args.plot_ig or args.plot_stats:
                    ...
                    observation_tensor = dict_obs_to_tensor(observation, device)
                    ...
                    if args.plot_ig:
                        ...
                        # get gradients
                        guided_score_grads, guided_perf_grads = IG.generate_gradients(observation_tensor)
                        ...
                        added_image_score = prepare_sheet_for_render(added_image_score, plain_env.resz_score,
                                                                     plain_env.resz_score, transform_to_bgr=False)
                    ...
                    if args.plot_stats:
                        ...
                        values.append(value)
                        tempo_curve.append(plain_env.sheet_speed)
                        ...
                        speed_bgr = get_opencv_bar(plain_env.sheet_speed, bar_heigth=bar_height, min_value=-15, max_value=15,
                                                   color=(255, 0, 255), title="speed")

                        # get tracking error bar
                        error_bgr = get_opencv_bar(np.abs(plain_env.tracking_error),
                                                   bar_heigth=bar_height, max_value=config['score_shape'][-1] // 2,
                                                   color=(0, 0, 255), title="error")

                        # get score progress bar
                        score_bgr = get_opencv_bar(episode_reward, bar_heigth=bar_height, max_value=plain_env.max_timesteps,
                        ...
                        pot_score_bgr = get_opencv_bar(len(tempo_curve), bar_heigth=bar_height,
                                                       max_value=plain_env.max_timesteps,
    ...
    # write video
    render_video(observation_images, env.unwrapped, fps=config['spectrogram_params']['fps'])
